...
# ใส่ Link .zip จาก GitHub (อย่าลืมใส่ <TOKEN>@ ถ้าเป็น Private Repo)
...
if [ ! -s "$DOWNLOAD_DEST" ]; then
    echo "❌ Error: Failed to download from GitHub. Please check the URL." | tee -a "$LOG_FILE"
    exit 1
fi
echo "✅ Download successful! Saved temporarily at: $DOWNLOAD_DEST" | tee -a "$LOG_FILE"

echo "-----------------------------------------------------------------" | tee -a "$LOG_FILE"
echo "🔍 Step 2: Searching for the downloaded ZIP file..." | tee -a "$LOG_FILE"

# ใช้คำสั่ง find ค้นหาไฟล์ที่เพิ่งโหลดมา (ค้นหาแค่ความลึกระดับ 2 เพื่อความรวดเร็ว)
ZIP_FILE=$(find "$USER_HOME" -maxdepth 2 -type f -name "$SEARCH_FILENAME" 2>/dev/null | head -n 1)

if [ -z "$ZIP_FILE" ]; then
    echo "❌ Error: ไม่พบไฟล์ $SEARCH_FILENAME ในระบบหลังจากดาวน์โหลด" | tee -a "$LOG_FILE"
    exit 1
fi
echo "✅ Found ZIP file confirmed at: $ZIP_FILE" | tee -a "$LOG_FILE"
    ...
    if [ -d "$SITE_PATH" ]; then
        ...
        if [ -n "$DOMAIN" ]; then
            ...
            if wp plugin is-installed "$PLUGIN_SLUG" --allow-root 2>/dev/null; then
                ...
                # ลบปลั๊กอินตัวเก่าทิ้งก่อน (แก้ปัญหาโฟลเดอร์ GitHub ชื่อไม่ตรง)
                wp plugin delete "$PLUGIN_SLUG" --allow-root >> "$LOG_FILE" 2>&1
                
                # สั่งติดตั้งจากไฟล์ ZIP ที่ค้นหาเจอ
                if wp plugin install "$ZIP_FILE" --activate --allow-root >> "$LOG_FILE" 2>&1; then
                    
                    # --- จัดการปัญหาชื่อโฟลเดอร์ของ GitHub (ลบขีดต่อท้ายออก) ---
...
# ลบไฟล์ ZIP ทิ้งหลังจากติดตั้งเสร็จทุกเว็บ
rm -f "$ZIP_FILE"

# --- 5) การนับยอดหลังทำงานเสร็จ (Post-Scan Summary) ---
echo "-----------------------------------------------------------------" | tee -a "$LOG_FILE"
echo "✅ Installation Process Completed!" | tee -a "$LOG_FILE"
echo "🗑️ Temporary ZIP file ($SEARCH_FILENAME) removed." | tee -a "$LOG_FILE"
echo "-----------------------------------------------------------------" | tee -a "$LOG_FILE"
